<?                
    class menuModel
    {
        ...
        function create()
        {
            return db::query("INSERT INTO menu (menu_name,class_name,auth_level,menu_order,menu_parent,menu_target,level_only) VALUES ('$this->menu_name','$this->class_name',0$this->auth_level,0$this->menu_order,0$this->menu_parent,'$this->menu_target',0$this->level_only);");
        }
        
        function read($id)
        {
            $menu = db::query("select * from menu where menu_id = $id");     
        ...
        function readAll()
        {
            $this->list = db::query('select * from menu');
        }        
    }
?>